...
**The batch entry point also RECORDS its result (#1434), and it is the only one that does.** Its input carries an optional `recipeId` and an optional per-item `ingredientId` — identity for persistence, read by nothing in the matcher — and when `recipeId` is present the flow folds `canonId`/`matchState` onto `recipes/{recipeId}` in a transaction after the results are assembled. Present, the answer is `{ results, persistence }` so the caller learns whether that fold landed (#1601); absent, it behaves exactly as before and answers the bare results array: `assembleRecipeDraft` runs this flow in-process for a recipe that is not in Firestore yet. Nothing about the matching changes in either arm; see `docs/recipe-module.md` § "Canon interaction" for the write's shape and its LWW limit.
