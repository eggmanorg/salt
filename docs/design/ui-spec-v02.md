# Salt 2.0 — UI Primitives Specification (v0.2.20)
...
The spec is versioned `vMAJOR.MINOR.PATCH` (currently v0.2.20).
...
**One treatment, and a page writes nothing to get it.** A base `:focus-visible`
rule in `salt.css` gives every focusable element the ring automatically:

```css
outline: 2px solid var(--salt-focus-color, hsl(var(--salt-ring)));
outline-offset: 2px;
```

`--salt-ring`, never `--salt-border`: `--salt-input` is byte-identical to
`--salt-border`, so a border-coloured ring drawn against a bordered control is
not an indicator at all — it is the same grey, 2px thicker. The `2px` gap is
what separates the ring from the control's own border, and from the row border
of anything nested. Never use `focus:` without `-visible`.

Three utilities restate it where a base rule cannot reach. They are the **only**
sanctioned way to write a focus ring; anything else is the drift #1305 removed.

| Utility                  | When                                                                                                                                                                                                                                                                                               |
| ------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `salt-focus-ring`        | The base rule is suppressed on this element (`.salt-input--combobox`, a value chip), or the focus target is not the element the ring should surround.                                                                                                                                              |
| `salt-focus-ring-within` | A **frame** owning the ring for a raw `<input>`/`<textarea>` inside it — `focus-within`, which no base rule can express (§8.2).                                                                                                                                                                    |
| `salt-focus-ring-inset`  | A target that **fills its container** — a row's `flex-1` button, a pane that is its own scroll region. The gap is spent inwards (`outline-offset: -2px`), because an outward ring there crosses the container's border, or sits outside an ancestor's `overflow-hidden` and is clipped to nothing. |
| `salt-focus-gutter`      | A **scroll container** holding a full-bleed focusable child. Not a ring — the 4px of `padding` and `scroll-padding` the ring needs in order to be drawn at all (below).                                                                                                                            |

**A scroll region owes the ring room.** The outline is painted outside the
border box, so the ring reaches 4px out — 2px of offset, then 2px of stroke. A
scrollport clips at its _padding_ box. A full-bleed control inside a scroll
container with no padding therefore has its ring sliced off flush along the
edge, on a control that looks correct the moment the scroller is taken away, and
the page's own padding buys it nothing because that padding sits outside the
scroller. `salt-focus-gutter` supplies the 4px, on both axes whatever the
container scrolls — CSS computes the other axis to `auto` when one is not
`visible`, so an `overflow-y-auto` column clips horizontally too — and the same
4px of `scroll-padding`, so keyboard focus does not park the ring half under the
edge it just scrolled from.

This one is applied by reading the page, not by a rule: nothing can tell which
scroll containers hold a full-bleed focusable child. What is mechanical is that
the gutter equals the ring's reach, so the two cannot drift.

**Retinting, not restating.** A state that needs a different ring colour sets
`--salt-focus-color` and changes nothing else — `.salt-input--error` is the only
wearer today. A component-layer class can never out-specify a utility-layer one,
so `focus-within:outline-destructive` on `.salt-input--error` would silently
lose; the custom property inherits and sidesteps the cascade.

**Forbidden:** `focus-visible:ring-*` / `focus-within:ring-*` anywhere in
`apps/**`; `outline-none` without something else drawing the ring in its place;
a second `:focus-visible` rule in an app's own CSS (the Storybook canvas had one
for months and it is why the app and its own canvas disagreed). Enforced by
`apps/web-pwa/tests/focusRingGuard.test.ts` and the focus-ring block in
`packages/ui-components/tests/tokens.theme.test.ts`.
...
frame error: 'salt-input--error'   /* border-destructive + --salt-focus-color retint; §4.2 */
...
| ---------- | ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 2026-09-08 | v0.2.20 | §4.2 Focus Ring, §8.2 TextField: the focus ring is **rewritten to what ships**, and what ships is corrected to what §4.2 always intended. §4.2 had described a `ring-*` box-shadow expansion for `salt-focus-ring` — teal, `ring-offset-2` — that the code never implemented: the shipped base rule and both utilities used `--salt-border` at `outline-offset-0`. Since `--salt-input` is byte-identical to `--salt-border`, that ring was invisible against any bordered control, and fifteen surfaces in `apps/web-pwa` had independently hand-written the spec-correct teal one, most first switching the wrong default off with `outline-none`. The Storybook canvas carried a fourth copy — the correct one — so the components focused right in the canvas and wrong in the app. §4.2 now states the one definition (`--salt-focus-color` defaulting to `--salt-ring`, 2px, `outline-offset: 2px`), the three utilities that restate it where a base rule cannot reach (`salt-focus-ring`, `-within`, and the new `-inset` for targets that fill their container), the retinting rule that replaces per-state restatement, and an explicit Forbidden list. §8.2's `frame error` line becomes `salt-input--error`: it was `focus-within:ring-destructive`, a Tailwind ring colour with no ring width on an element whose ring is an outline, so an errored field had no focus indicator at all. No primitive's props changed; nothing re-stamped. Adds `salt-focus-gutter`, the 4px of `padding`/`scroll-padding` a scroll container owes a full-bleed focusable child: an outline is painted outside the border box and a scrollport clips at its padding box, so the catalog's two panes and the recipe view's method column sliced the new ring off along every edge. Enforced by `apps/web-pwa/tests/focusRingGuard.test.ts` and the focus-ring block in `tokens.theme.test.ts` (CLAUDE.md rule 12). Issue #1305.                      |
