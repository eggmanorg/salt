...
consuming app's — `kitchenToolService`'s `toolPicture` today — and it lives in
`apps/web-pwa` because it reads a Firestore-backed vocabulary. A primitive that resolved names would need that vocabulary, which is
