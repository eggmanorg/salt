...
- **A nested table survives the import as readable prose, not as markup.** Both
  tables collapse to their cells' text run together with no separator at all.
  The no-markup property holds; readability does not. What a nested table
  _should_ become is a product question with no answer yet.
