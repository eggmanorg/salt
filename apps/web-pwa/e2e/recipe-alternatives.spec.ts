/**
 * "Chef's Specials" — specials end to end (issue #637, Phase 3).
 *
 * A special is a takeaway, a picnic, a meal out or a fend-for-yourself night: it
 * fills a planner slot but has no ingredients and no method. This walks the one
 * journey that has to work — New → Chef's Specials → title + description → save —
 ...
 *
 * Phase 4 adds the second journey: a special is PICKABLE in the meal planner —
...
const SPECIAL_TITLE = 'Takeaway — Indian';
const RECIPE_TITLE = 'Reference Recipe';
const RECIPE_ID = 'alternatives-reference-recipe';
const SEEDED_SPECIAL_ID = 'alternatives-planned-special';
...
// including `/recipes/new/special`, the one a special used to be typed into. What
...
test.describe("recipes — chef's specials", () => {
  test('create a special, find it under its own section, and not among the recipes', async ({
    page,
  }, testInfo) => {
    ...
    // Bridge-seeded. The special below is still CREATED, through the New sheet,
    ...
    await page.getByTestId('recipe-new-name').fill(SPECIAL_TITLE);
    ...
    await expect(page.getByRole('heading', { name: SPECIAL_TITLE })).toBeVisible({
    ...
    await expect(cards.filter({ hasText: SPECIAL_TITLE })).toHaveCount(0);

    // Switching sections swaps one for the other. "Chef's Specials" is a secondary
    // section, so the row has to be expanded before its chip exists.
    await page.getByTestId('recipe-kind-show-all').click();
    await sections.getByRole('button', { name: "Chef\'s Specials" }).click();
    await expect(cards.filter({ hasText: SPECIAL_TITLE })).toHaveCount(1);
  ...
  test("plan a chef's special night: it is offered in the picker, fills the day, and asks for no shopping", async ({
    page,
  }, testInfo) => {
    // Single-tab and no AI/trigger wait, but two separate document round-trips
    // (the seeded special, then the meal-plan week) plus a route change — the same
    ...
    // ── Seed the special ──────────────────────────────────────────────────────
    // Bridge-seeded. It was authored through `/#/recipes/new/special` so that the
    ...
    // an entry of kind `special` is offered, fills a day, and asks for no shopping.
    const specialId = SEEDED_SPECIAL_ID;
    await seedRecipe(page, entry(specialId, SPECIAL_TITLE, 'special'));
    ...
    // ── The special is offered, alongside the recipes ─────────────────────────
    ...
    // title plus its "Chef's Specials" badge, hence the substring match.
    await page.getByTestId(`${testid}-recipe-picker`).click();
    const option = page.getByRole('option', { name: new RegExp(SPECIAL_TITLE) });
    await expect(option).toBeVisible({ timeout: SYNC_TIMEOUT });
    // The badge is what tells it apart from a recipe at a glance.
    await expect(option).toContainText("Chef's Specials");
    await option.click();
    ...
    await expect
      ...
      .toEqual({ recipeIds: [specialId], note: SPECIAL_TITLE });
    ...
    await expect(page.getByTestId(`${testid}-recipe-row-${specialId}`)).toBeVisible({
      timeout: SYNC_TIMEOUT,
    });
    await expect(page.getByTestId(`${testid}-recipe-remove-${specialId}`)).toBeVisible();
    await expect(page.getByTestId(`${testid}-recipe-addshop-${specialId}`)).toHaveCount(0);
  });
});
