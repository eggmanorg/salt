...
describe('RecipeViewPage — unreviewed import', () => {
  ...
  // Done is the deliberate act that says "I have read this" (issue #1319), and
  // it routes through the very same `handleMarkReviewed` the banner's own button
...
describe('RecipeViewPage — edit mode', () => {
  it('offers Edit but nothing editable until it is pressed', () => {
    mockRecipes._set([makeRecipe({ description: 'Silky' })]);
    const { getByTestId, queryByTestId } = renderPage();
    ...
    expect(queryByTestId('recipe-edit-description')).toBeNull();
  });
  ...
  it('puts the pencils away again on Done', async () => {
    ...
    await fireEvent.click(getByTestId('recipe-edit-mode-button'));
    expect(getByTestId('recipe-edit-description')).toBeTruthy();

    await fireEvent.click(getByTestId('recipe-done-button'));

    await waitFor(() => expect(queryByTestId('recipe-edit-description')).toBeNull());
    expect(getByTestId('recipe-cook-button')).toBeTruthy();
  });
