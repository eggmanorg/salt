...
- **Issues are filed through `/salt-spec`, `/salt-defect`, `/salt-refactor` or `/salt-epic`** — never a free-form `gh issue create`. Only the first three shapes are executable by `/salt-run`, and `scripts/check-spec-shape.mjs` decides which body qualifies (it gates the `specced` label). Those commands are user-invoked, so an agent needing an issue mid-flight spawns a subagent pointed at `.claude/commands/salt-<cmd>.md`. Writing the body yourself is the fallback, and then you verify it with that script. An **epic** is a container, never a work issue: `/salt-epic` files one, it can carry no `## Phases`, and it is refused `specced` on its title alone. Departing from this takes a stated reason.
